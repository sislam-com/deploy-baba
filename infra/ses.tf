...
resource "aws_route53_record" "ses_dkim" {
  ...
  name    = "${tolist(aws_sesv2_email_identity.mail.dkim_signing_attributes)[0].tokens[count.index]}._domainkey.${local.ses_domain}"
  type    = "CNAME"
  ttl     = 600
  records = ["${tolist(aws_sesv2_email_identity.mail.dkim_signing_attributes)[0].tokens[count.index]}.dkim.amazonses.com"]
}
